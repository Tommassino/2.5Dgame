...
public class TGALoader {
	...
	public static void loadTGA(Texture texture, String filename)
		...
		if (uTGAcompare.equals(header)) { // an Uncompressed TGA image
											// If so, jump to Uncompressed TGA
											// loading code
	...
	{ // TGA Loading code nehe.gamedev.net)
		...
		if (texture.bpp == 24) // If the BPP of the image is 24...
			texture.type = GL11.GL_RGB; // Set Image type to GL_RGB
		else
			// Else if its 32 BPP
			texture.type = GL11.GL_RGBA; // Set image type to GL_RGBA
		...
		texture.imageData = ByteBuffer.allocateDirect(tga.imageSize);
	...
	private static void loadCompressedTGA(Texture texture,
			ReadableByteChannel fTGA) throws IOException // Load COMPRESSED TGAs
	{
		TGA tga = new TGA();
		...
		// Storage for 1 pixel
		ByteBuffer colorbuffer = ByteBuffer.allocateDirect(tga.bytesPerPixel);

		do {
			int chunkheader; // Storage for "chunk" header
			try {
				ByteBuffer chunkHeaderBuffer = ByteBuffer.allocateDirect(1);
			...
			if (chunkheader < 128) { // that follow the header
				...
				for (short counter = 0; counter < chunkheader; counter++) { // by
																			// the
																			// header
					texture.imageData.put(currentbyte, colorbuffer.get(2));
					texture.imageData.put(currentbyte + 1, colorbuffer.get(1));
